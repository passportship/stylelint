...
module.exports = function (stylelint, options = {}) {
	...
	return getCode
		.then((code) => {
			...
			if (!isAutoSyntax && syntax && syntax.stringify) {
				/**
				 * @param {import("postcss").Root | import("postcss").AtRule | import("postcss").Rule | import("postcss").Declaration | import("postcss").Comment} obj
				 */
				let changeToString = (obj) => {
					// @ts-ignore
					if (!obj.__proto__.nativeToString) {
						// @ts-ignore
						obj.__proto__.nativeToString = obj.__proto__.toString;
						// @ts-ignore
						obj.__proto__.toString = function () {
							return this.nativeToString(syntax);
						};
					}
				};

				if (result && result.root) {
					changeToString(result.root);

					if (result.root.nodes) {
						for (const obj of result.root.nodes) {
							changeToString(obj);
						}
					}
				}
